...
```mermaid
flowchart LR
    CUN01["CUN-01<br/>Despachar Cargas Mono SKU desde CD"] --> CUN02["CUN-02<br/>Trasladar Cargas hacia Sedes"]
    CUN02 -->|Camino Principal| CUN03["CUN-03<br/>Entregar Carga en Sede Autorizada"]
    CUN02 -->|Camino Alternativo| CUN04["CUN-04<br/>Gestionar Rechazo o Retorno"]
    CUN03 & CUN04 -.->|Sincronización Transversal (ACT-12 / RF-10)| CUN05["CUN-05<br/>Consultar Trazabilidad y Estado"]
```
...
| **Flujo Normal (Básico):** | 1. El Supervisor de Despacho recibe físicamente las cajas embaladas en la Zona de Despacho (`ACT-01`).<br/>2. El Supervisor consulta en el sistema Driving la dirección de destino del pedido y clasifica el pedido según su canalización geográfica en los 24 departamentos (`ACT-02`).<br/>3. El Supervisor asigna al pedido el socio logístico contratado para esa ruta y registra la modalidad de envío aplicable: terrestre, bimodal o aérea (`ACT-03`).<br/>4. El Supervisor de Despacho asocia la promesa estimada de entrega según la región geográfica de destino (24 horas para Lima Metropolitana y hasta 7 días para provincias), con el soporte del sistema NSDG/Driving (`ACT-04`).<br/>5. El Supervisor entrega formalmente las cajas al transportista asignado y registra el egreso en los sistemas Driving/NSDG, formalizando la transferencia de custodia física y actualizando el estado a *"Despachado"* (`ACT-05`), concluyendo el caso de uso del negocio. |
| **Flujos Alternativos:** | No se presentan flujos alternativos en el alcance operativo sustentado del AS-IS. |
| **Postcondiciones:** | - La custodia física del paquete queda formalmente bajo responsabilidad del transportista.<br/>- El estado del pedido queda actualizado a **"Despachado"** en NSDG/Driving.<br/>- Quedan registrados: socio logístico, modalidad de envío, estampa de tiempo y promesa de entrega.<br/>- El egreso formal queda registrado en los sistemas locales y disponible para la actividad técnica transversal de sincronización (`ACT-12` / `RF-10`). |
...
| **Flujo Normal (Básico):** | 1. El Socio Logístico confirma la recepción de la carga y registra en el sistema de transporte (NSDG/Driving) el inicio de la ruta (`ACT-06`), actualizándose formalmente el estado del pedido a *"En Ruta"*.<br/>2. El transportista traslada físicamente la carga por el trayecto asignado según su modalidad (`ACT-07`): terrestre, bimodal o aérea.<br/>3. La unidad arriba a la localidad o domicilio de destino dentro del plazo estimado (24h Lima / hasta 7 días provincias) para dar inicio a la última milla (`CUN-03` o `CUN-04`), concluyendo la etapa de traslado en tránsito. |
| **Flujos Alternativos:** | No se registran flujos alternativos durante el tránsito regular. Si durante el traslado se suscita una contingencia tipificada (retraso, pérdida o daño) que impida concretar la entrega, el proceso se bifurca hacia el camino alternativo CUN-04. |
| **Postcondiciones:** | - El pedido se mantiene en tránsito con estado formal **"En Ruta"**.<br/>- La carga arriba físicamente a la localidad de entrega para dar inicio a la última milla (`CUN-03` o `CUN-04`).<br/>- El registro del inicio de traslado queda disponible en los sistemas de transporte para la actividad técnica transversal de actualización (`ACT-12` / `RF-10`). |
...
### CUN-03: Entregar Carga en Sede Autorizada de Destino

| Campo | Especificación del Estándar UTP / RUP |
|:---|:---|
| **Código y Nombre:** | **CUN-03 — Entregar Carga en Sede Autorizada de Destino** |
| **Actores Participantes:** | - **Socio Logístico / Transportista** *(«Business Actor» — Ejecutor de la entrega en sede y responsable del registro)*<br/>- **Consultora / Distribuidor de Yanbal** *(«Business Actor» — Destinatario titular de la carga)*<br/>- **Personal Autorizado (en Sede / Agencia)** *(«Business Actor» — Receptor o encargado autorizado en sede de destino)* |
| **Descripción:** | Camino operativo principal en destino mediante el cual el transportista se presenta en la sede o agencia autorizada de destino, efectúa la entrega física de la carga consolidada mono SKU al titular o al personal autorizado presente en la sede, captura los datos del receptor real (identidad, consignación y firma) y registra la confirmación de la entrega, actualizando el estado de la carga a *"Entregado"*. |
| **Precondiciones:** | 1. La carga se encuentra en estado *"En Ruta"* (`CUN-02`) y la unidad de transporte ha llegado a la sede o agencia autorizada de destino.<br/>2. Existe un receptor calificado presente en la sede (responsable titular o personal autorizado de recepción). |
| **Flujo Normal (Básico):** | 1. El transportista arriba a la sede o agencia autorizada de destino e interactúa con el responsable de recepción.<br/>2. El **responsable titular** se identifica como receptor de la carga.<br/>3. El transportista entrega físicamente las cajas mono SKU consolidadas.<br/>4. El transportista registra en la herramienta de campo de NSDG la confirmación de la entrega (`ACT-08`), capturando que el receptor fue el titular (`ACT-09`).<br/>5. El estado de la carga se actualiza formalmente a **"Entregado"**, concluyendo la entrega física y el caso de uso del negocio. |
| **Flujos Alternativos:** | **2.a. Recepción por Personal Autorizado en Sede (Escenario Crítico PR-05):**<br/>- 2.a.1. El titular no se encuentra disponible al momento de la llegada.<br/>- 2.a.2. El personal autorizado en sede se identifica ante el transportista como encargado designado de recepción.<br/>- 2.a.3. El transportista entrega la carga al personal autorizado y registra la entrega en NSDG (`ACT-08`), consignando los datos del personal autorizado como receptor real (`ACT-09`).<br/>- 2.a.4. El flujo concluye en el paso 5 con estado *"Entregado"*.<br/><br/>**2.b. Imposibilidad de entrega por contingencia o rechazo (Bifurcación hacia CUN-04):**<br/>- 2.b.1. Si en la llegada a sede se suscita una contingencia tipificada exclusivamente por la matriz (**retraso, pérdida o daño**), o la carga es rechazada por daño físico, el transportista no concluye la entrega y el proceso continúa por el camino alternativo `CUN-04`. |
| **Postcondiciones:** | - La carga física queda en posesión de la sede o agencia autorizada de destino.<br/>- El estado de la carga cambia formalmente a **"Entregado"** en los sistemas de transporte.<br/>- Queda registrada la condición de quien recibió físicamente (titular o personal autorizado en sede).<br/>- El evento de entrega y los datos del receptor quedan registrados en los sistemas de transporte y disponibles para la actividad técnica transversal de actualización (`ACT-12` / `RF-10`), quedando sujetos en el AS-IS al desfase de sincronización de hasta 2 horas hacia los canales de consulta (problema PR-03). |
| **Reglas del Negocio:** | - **RN-06 (Recepción Válida en Sede / Agencia):** Se autoriza la entrega física al titular o al personal autorizado designado en la sede o agencia de destino.<br/>- **RN-07 (Captura de Condición de Receptor):** El transportista es el responsable operativo exclusivo de registrar si quien recibió la carga fue el titular o personal autorizado en sede. El receptor presencial no realiza registros en el sistema del transportista. |
| **Trazabilidad:** | - **Requerimientos Funcionales:** [[02 - Especificación de Requerimientos Funcionales#RF-06\|RF-06]], [[02 - Especificación de Requerimientos Funcionales#RF-07\|RF-07]].<br/>- **Actividades AS-IS:** [[03 - Actividades y eventos#ACT-08\|ACT-08]], [[03 - Actividades y eventos#ACT-09\|ACT-09]].<br/>- **Requerimientos No Funcionales:** [[03 - Especificación de Requerimientos No Funcionales#RNF-06\|RNF-06]], [[03 - Especificación de Requerimientos No Funcionales#RNF-07\|RNF-07]].<br/>- **Problemas AS-IS:** [[05 - Problemas y desfases#PR-05\|PR-05]] (Receptor real no comunicado oportunamente). |
...
| **Flujo Normal (Básico):** | 1. El Socio Logístico constata la contingencia tipificada en ruta o en destino.<br/>2. El transportista registra la entrega fallida en el sistema de transporte (NSDG/Driving), tipificando la causa exclusivamente como **retraso, pérdida o daño** (`ACT-10`).<br/>3. El estado del pedido se actualiza formalmente a **"Entrega Fallida"**.<br/>4. Al contar con el bulto físico no entregado (en causales de retraso o daño), el transportista registra el inicio de retorno del pedido hacia el Centro de Distribución (`ACT-11`).<br/>5. Concluye el caso de uso del negocio al quedar formalizado el inicio del retorno de la carga no entregada. |
| **Flujos Alternativos:** | **2.a. Pérdida de mercancía en ruta (sin bulto físico):**<br/>- 2.a.1. En caso de pérdida de la carga durante el transporte, el transportista registra la entrega fallida en NSDG/Driving tipificando exclusivamente la causal *pérdida* (`ACT-10`).<br/>- 2.a.2. Al no existir bulto físico que retornar, no se realiza ni se afirma registro de inicio de retorno físico (`ACT-11` no aplica en este escenario), concluyendo formalmente la gestión de la incidencia en campo. |
| **Postcondiciones:** | - El estado del pedido queda registrado formalmente como **"Entrega Fallida"** en los sistemas de transporte, asociado a la causal tipificada (retraso, pérdida o daño, según `ACT-10` / `RF-08`).<br/>- Según el escenario de la contingencia constatada:<br/>  a) **En caso de retorno físico (retraso o daño con bulto presente):** queda formalmente registrado el inicio del retorno de la carga hacia el Centro de Distribución por el socio logístico (`ACT-11` / `RF-09`).<br/>  b) **En caso de pérdida (sin bulto físico):** queda formalmente registrada la incidencia de entrega fallida por pérdida en los sistemas; no se realiza ni se afirma registro de retorno físico de carga por no existir bulto que retornar.<br/>- La información registrada queda disponible en los sistemas para la actividad técnica transversal de actualización (`ACT-12` / `RF-10`). |
...
| **Flujo Normal (Básico):** | 1. El actor (Consultora o Agente de Servicio al Cliente) realiza la consulta de la situación del pedido indicando el identificador de búsqueda: **Número de Pedido** o **Código de Consultora** (`ACT-13`).<br/>2. El consultante obtiene la situación registrada del pedido: su estado actual de distribución (*Despachado, En Ruta, Entregado, Entrega Fallida*) y la promesa estimada de entrega (`ACT-13`).<br/>3. **Condición actual del AS-IS (Problemas PR-03 y PR-04):** La información obtenida refleja la última actualización transmitida desde campo, la cual presenta un desfase de hasta 2 horas respecto a los eventos reales debido a la latencia en la propagación de estados (`ACT-12` / `RF-10`).<br/>4. **Problemática de visibilidad del receptor real (Problema PR-05, Necesidad RF-12 y Actividad ACT-14):**<br/>   - *Situación actual (AS-IS):* Durante la entrega física (`CUN-03`), el transportista registra al receptor (`ACT-09`), pero dicha información no se encuentra disponible oportunamente en los canales de consulta web ni CRM.<br/>   - *Problema (PR-05):* Cuando el pedido es recibido por una persona autorizada en el domicilio, la consultora no dispone de esa información en el sistema, lo que genera incertidumbre y reclamos falsos de no-entrega.<br/>   - *Necesidad/Requerimiento (RF-12):* Representa la necesidad funcional de visualizar los datos del receptor real en los canales de atención y seguimiento.<br/>   - *Actividad (ACT-14):* Actividad de visualización del receptor real en canales de atención y seguimiento, vinculada a `CUN-05`, cuya disponibilidad oportuna en pantalla se encuentra limitada en el AS-IS por el problema PR-05. |
| **Flujos Alternativos:** | **3.a. Consulta durante la ventana de desfase de actualización (~2 horas) (Manifestación de PR-03 y PR-04):**<br/>- 3.a.1. El pedido fue físicamente entregado en campo, pero al consultarse figura aún como "En Ruta" debido al tiempo de propagación de hasta 2 horas.<br/>- 3.a.2. La consultora contacta a Servicio al Cliente, donde el agente visualiza la misma información desactualizada ("En Ruta"), viéndose imposibilitado de confirmar la entrega en vivo.<br/><br/>**4.a. Falta de visibilidad de entrega a persona autorizada (Manifestación de PR-05):**<br/>- 4.a.1. El pedido fue recibido por una persona autorizada en domicilio, pero dicha condición de recepción no se visualiza oportunamente en la consulta.<br/>- 4.a.2. La consultora presume que su mercancía no ha llegado e inicia un reclamo ante el agente de atención al cliente. |
...
| **Reglas del Negocio:** | - **RN-10 (Criterios de Búsqueda Homologados):** La consulta admite indistintamente la búsqueda por Número de Pedido o Código de Consultora (`RF-11`).<br/>- **RN-11 (Necesidad Operativa de Identificación del Receptor - RF-12):** En el AS-IS se constata la necesidad operativa de identificar y mostrar la información del receptor real (persona autorizada) en la consulta (`ACT-14` / `RF-12`) para prevenir reclamos infundados de no-entrega (**PR-05**). |
