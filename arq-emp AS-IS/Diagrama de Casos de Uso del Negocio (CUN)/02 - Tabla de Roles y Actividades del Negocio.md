...
|:---|:---:|:---|:---:|:---:|:---|:---|
| **Consultora / Distribuidor de Yanbal** | «Business Actor»<br/>*(Actor del Negocio)* | Destinatario en la cadena de distribución. Recibe la carga consolidada o gestiona el retiro en sede/agencia (`CUN-03`), consulta la situación y promesa de entrega (`ACT-13`) y visualiza la información de entrega en el portal web (`ACT-14`), experimentando en el AS-IS la limitación de visibilidad de `PR-05`. | [[03 - Actividades y eventos#ACT-13\|ACT-13]], [[03 - Actividades y eventos#ACT-14\|ACT-14]]<br/>*(Receptora presencial en CUN-03; el registro operativo en sistema corresponde al transportista)* | [[01 - Diagrama General de Casos de Uso del Negocio (CUN)#CUN-03\|CUN-03]], [[01 - Diagrama General de Casos de Uso del Negocio (CUN)#CUN-05\|CUN-05]] | Portal Web / Canal de Tracking | Líneas 21-22, 24-25<br/>(min 22:03–22:28, 24:45) |
| **Personal Autorizado (en Sede / Agencia)** | «Business Actor»<br/>*(Actor del Negocio)* | Personal designado o autorizado en la sede o agencia de destino que recepciona y consigna la entrega física de la carga consolidada. Recibe físicamente los bultos e interactúa con el transportista. No realiza registros en el sistema del transportista. | [[03 - Actividades y eventos#ACT-09\|ACT-09 (Receptor en sede)]]<br/>*(Receptor presencial en CUN-03; la captura operativa en sistema corresponde al transportista)* | [[01 - Diagrama General de Casos de Uso del Negocio (CUN)#CUN-03\|CUN-03]] | Interacción física en sede de destino | Línea 25<br/>(min 24:45–25:01) |
...
### Ficha 1: Consultora / Distribuidor de Yanbal
- **Clasificación RUP:** «Business Actor» (Actor del Negocio externo).
- **Misión Operativa:** Destinatario en la cadena de distribución. Recibe físicamente la carga consolidada o gestiona el retiro en sede/agencia y consulta el estado de seguimiento.
- **Participación en los CUN:**
  - `CUN-03 (Entregar Carga en Sede Autorizada de Destino):` Receptora o titular en destino (participación física presencial; el registro operativo en sistema es realizado por el transportista).
  - `CUN-05 (Consultar Trazabilidad y Estado de Despacho):` Consultante de la situación de la carga mediante Número de Pedido o Código de Consultora (`ACT-13`), y visualizadora de la información disponible sobre la entrega (`ACT-14`).
- **Impacto de Problemas AS-IS:** Padece **PR-03** (información desfasada por hasta 2 horas en el portal) y **PR-05** (al no ser notificada oportunamente si recibió un personal autorizado en sede/agencia, presume no-entrega o pérdida de su carga).

### Ficha 2: Personal Autorizado (en Sede / Agencia)
- **Clasificación RUP:** «Business Actor» (Actor del Negocio externo).
- **Misión Operativa:** Personal designado o encargado en la sede o agencia de destino que actúa como receptor formal cuando recepciona y consigna la entrega física de la carga consolidada mono SKU.
- **Participación en los CUN:**
  - `CUN-03 (Entregar Carga en Sede Autorizada de Destino):` Recibe físicamente la carga consolidada en la sede o agencia e interactúa con el transportista (`ACT-09`, consignación y firma física; el registro operativo es ejecutado exclusivamente por el transportista).
- **Impacto de Problemas AS-IS:** Centro del problema **PR-05**: su recepción física en sede es válida, pero la falta de visibilidad oportuna de sus datos de recepción en los canales de consulta genera incertidumbre y falsos reclamos.
...
- **Impacto de Problemas AS-IS:** Padece directamente **PR-04** (margen de desconocimiento de hasta 2 horas provocado por la latencia en la actualización) y **PR-05** (falta de visibilidad oportuna de la identidad del receptor real cuando recibió una persona autorizada).
...
| **ACT-09** | Registrar datos de la persona que recibe el paquete | Socio Logístico *(Actor)*<br/>*(Receptor: Consultora o Persona Autorizada)* | NSDG | `CUN-03` | Captura de la condición de quién recibió en destino (titular o persona autorizada). |
