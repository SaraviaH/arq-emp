...
4. Posee trazabilidad bidireccional contra los **28 Requerimientos Funcionales** activos del sistema (RF001 a RF028).
...
│ • Independiente de la UI.      │ • Se derivan de los 29 Requerimientos Funcionales (RF).        │
...
* **El CUS** modelará cómo el conductor presiona un botón en la App Nativa para transmitir telemetría, cómo el supervisor genera un token de activación de 8 caracteres o cómo el sistema publica un evento al Bus corporativo en menos de 30 minutos.
...
sequenceDiagram
    ...
    participant CLOUD as Base de Datos (Logs) (logs y GPS)
    ...
    CLOUD-->>SAC: 3. Visualiza registro de guía sellada y estampa GPS
...
3. **Validación de Evidencias Certificadas:** Accede a coordenadas GPS atómicas y registros de comprobación (guías selladas o pallets estibados) alojadas de forma segura en la nube.
...
| :--- | :--- | :--- | :---: |
| **Socio Logístico / Conductor** | **Usuario Móvil (App Nativa Android)** | Inicia sesión sin contraseña mediante Código de Activación efímero de 8 caracteres; transmite telemetría GPS cada 10 min en segundo plano; almacena datos en SQLite (Room) offline; confirma entrega conforme o rechazo de carga. | RF010 a RF021 |
| **Supervisor de Distribución** | **Operador de Torre de Control (Web)** | Consulta despachos disponibles; habilita seguimiento y códigos; monitorea la flota mediante grilla operativa con semaforización visual; ejecuta cancelación forzada de seguimiento ante contingencia externa comunicada por llamada. | RF007 a RF009, RF022 |
| **Jefe de Distribución** | **Usuario Ejecutivo / Analítico (Web)** | Consume el Dashboard de KPIs (*Lead Time*, puntualidad, latencia de integración $\le$ 30 min); analiza el rendimiento comparativo por transportista y exporta consolidados exclusivamente en formato Excel (`.xlsx`). | RF024 |
| **Operador SAC / Soporte Logístico** | **Usuario de Consulta Rápida (Web)** | Utiliza el buscador de trazabilidad de despachos; visualiza el *timeline* cronológico completo en $<$ 2 s con coordenadas GPS, hitos y tiempos. | RF023 |
| **Punto de Destino / Agencia** | **Entidad Condicionante de Validación** | No requiere credenciales web propias (para evitar costos administrativos en 24 regiones). Afecta al sistema al validar o rechazar físicamente la carga, forzando la confirmación de entrega o rechazo tipificado por el conductor. | RF015, RF016, RF017 |
...
* **Defensa de los 5 Actores (1.5 minutos):**  
  ...
  > *5. El **Operador SAC / Soporte Logístico**, actor interno esencial que atiende reclamos en Salesforce y consulta evidencias de estado en menos de 2 segundos.*  
...
* **Cierre y Trazabilidad al Software (1 minuto):**  
  > *"Estos actores articulan **5 Casos de Uso del Negocio (CUN)**: Despacho en CD, Traslado Interprovincial, Gestión de Contingencias Viales, Entrega en Destino y Auditoría de Rendimiento.*  
  > *Es fundamental remarcar que este diagrama modela los procesos de la empresa y no pantallas de software. Cada CUN se encuentra mapeado bidireccionalmente contra nuestros **29 Requerimientos Funcionales**, los cuales darán soporte a los Casos de Uso del Sistema (CUS) en la siguiente entrega. Quedamos a su disposición para las preguntas."*
...
* **Respuesta Maestra:**  
  > *"En el minuto 24:09 de la entrevista, el Ing. Joao Condorpusa explicó textualmente que el retraso de 2 horas en el tracking impacta directamente en el área de Servicio al Cliente, porque las agencias y clientes reclaman sin que el operador sepa qué pasó con la carga. El Operador SAC interviene en el proceso **CUN-05** como el actor interno que consulta la línea de tiempo y valida las evidencias de entrega (logs y coordenadas GPS) para resolver disputas en el primer contacto sin tener que llamar por teléfono a los choferes en plena ruta."*
